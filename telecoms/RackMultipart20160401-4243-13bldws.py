...
strinh="HTTP/1.1 451 Unavailable For Legal Reasons\r\n Content-Type: text/html\r\n\r\n <h1> Site blocked<h1>"
strw="HTTP/1.1 418 Unavailable For Legal Reasons \n\rContent-Type: text/html\n\r <h1> Site blocked<h1>"
...
class Cache: 
    ...
    def insert(self,path,data,date):
        ...
        if path not in cache_line.keys():
            self.intialise_cacheline(path)
            ordered_keys.append([path,date])
        cache_line[path].append(data) 
        #print cache_line[path]
       

        #if self.cache_size<len(cache_line):
        #    self.least_recently_us()
    def remove(self,i):
        to_rm_path=ordered_keys.pop(i)
        del cache_line[to_rm_path[0]]

    def fetch(self,path):

        
        if path  in cache_line.keys():
            for i in range(len(ordered_keys)):

                if ordered_keys[i][0]==path:

                    
                    if ordered_keys[i][1]<datetime.datetime.now(): # if the cache is outdated 
                        print " outdated "
                        self.remove(i)
                        return False
                    else: 
                        ...
                        #data=ordered_keys.pop(i) ## reordering so that it does  LRU
                        #cache_line[path].append(data) 

                        return cache_line[path]
        else: 
            return False
